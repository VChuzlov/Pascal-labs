...
begin
  ...
  begin
    k1 := f(t, result[i-1], k);
    k2 := f(t + 2 / 2, sum(h / 2, result[i-1], k1), k);
    k3 := f(t + 2 / 2, sum(h / 2, result[i-1], k2), k);
    k4 := f(t + 2 / 2, sum(h / 2, result[i-1], k3), k);
...
begin
  ...
  var c := RK(equations, time, c0, k, 1.0);
 ...
 begin
  ...
  for var j := 0 to y[i].High do
   write(abs((c[i][j]- ca[j]) / ca[j]) * 100:8:4);
  writeln;
  x += 1.0
 end
end.              